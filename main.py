...
def build_dfg(expression: str, folder_path : str):
    ...
    roots = builder.build(ast_root)
    print("Build Done")
    
    dfg = visualize_dfg(builder.all_nodes)
    dfg.render(folder_path + "/pics/BuildDFG", format='png', view=False, cleanup=True)
...
def run_test(folder_path : str):
    ...
    dfg_roots = build_dfg(expression=data["Expression"], folder_path=folder_path)

    schedule_info = schedule_dfg(dfg_roots, algorithm=data["Algorithm"], config=data["Config"], folder_path=folder_path)
